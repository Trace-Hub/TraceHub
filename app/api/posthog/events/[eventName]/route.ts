...
export async function GET(
	...
	try {
		...
		const [totalCount, uniqueUsers, sessionCount] = (metricsResult
			.results[0] ?? [0, 0, 0]) as [number, number, number];
		// 이전 기간 — 컬럼 순서 [previousTotal, previousUniqueUsers, previousSessionCount]
		const [previousTotal, previousUniqueUsers, previousSessionCount] =
			(previousResult.results[0] ?? [0, 0, 0]) as [number, number, number];
